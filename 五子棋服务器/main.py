...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
         ...
         #判断输赢
        if iswin(black_chesses) and not is_recieve1 and not is_recieve3:
        ...
        if iswin(white_chesses) and not is_recieve1 and not is_recieve3:
            ...
            if pygame.mouse.get_pressed()[0]:
                pos = pygame.mouse.get_pos()
                if win_text_rect.left < pos[0] < win_text_rect.right and\
                   win_text_rect.top < pos[1] < win_text_rect.top + 120:
                    tcpclisock.send('again')
                if win_text_rect.left < pos[0] < win_text_rect.right -50 and\
                   win_text_rect.top + 50< pos[1] < win_text_rect.top + 120:
                    is_people = False
